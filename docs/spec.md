...
- `init` が既存設定ファイルの `id` を再利用する場合、
  ...
  既存鍵の供給を求めて失敗する。
- v1 の `init` は新しい `DOTENV_PRIVATE_KEY` を生成しない。
- `init` が取り込める鍵ソースは、解決した ID に対応する
  既存の OS ストア値、親プロセスの `DOTENV_PRIVATE_KEY`、
  またはプロジェクトルートに既存の `.env.keys` に限る。
...
6. 2 から 5 に該当しない場合は、
  新規鍵不足エラーとして失敗し、
  先に `dotenvx` 側で鍵を用意するか、
  承認済みの別経路から `DOTENV_PRIVATE_KEY` を供給するよう促す。
7. `.dotenvx-keychain` を `{ "id": "..." }` 形式で書き込む。
8. `.env.keys` が存在する場合は削除する。
9. 成功メッセージとして ID と設定ファイルの配置先を表示する。
...
- 新規ディレクトリで、ローカルストア、`DOTENV_PRIVATE_KEY`、`.env.keys` の
  ...
  非 0 終了になり、先に `dotenvx` 側で鍵を用意するよう案内される。
...
- `init` 自身が安全な隔離ブートストラップを使って
  新規鍵生成まで担うかどうかは、
  [designs/90-future-init-key-generation.md](./designs/90-future-init-key-generation.md)
  で将来検討事項として扱う。
