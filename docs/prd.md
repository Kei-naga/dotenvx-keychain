...
プロジェクトで利用可能な既存の復号鍵を OS のキーチェーンへ登録し、不要な `.env.keys` を即座に破棄する。新しい鍵生成や `.env` の暗号化自体は `dotenvx` 本体の責務とする。**成功時は識別子を記録した設定ファイルを出力する。**
...
- **挙動:** カレントディレクトリのパス（またはそのハッシュ・フォルダ名）から自動で識別子を算出し、利用可能な既存鍵をキーチェーンへ保存する。
...
- **挙動:** 指定された `my-app-v2` を識別子として、利用可能な既存鍵をキーチェーンに保存する。
- **ファイル出力:** プロジェクトルートに `.dotenvx-keychain`（内容: `{"id": "my-app-v2"}`）を生成する。

> **前提**
> `init` が取り込める鍵ソースは、既存のローカルキーチェーン、親プロセスの `DOTENV_PRIVATE_KEY`、または `dotenvx` が事前に生成した `.env.keys` とする。新規に暗号化を始める場合は、先に `dotenvx` 側で鍵を用意しておく。
