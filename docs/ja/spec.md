...
- ローカル開発環境向け CLI の `init`、`run`、`set`、`get`、`list`、`remove`
- `.dotenvx-keychain` 設定ファイルの形式と解決規則
- OS ネイティブのシークレットストアへの保存ルール
- bundled `dotenvx` の `run`、`set`、`get` 実行時に必要な鍵注入の振る舞い
...
- `run`、`set`、`get` はプロジェクトルート直下だけでなく、
  その子ディレクトリからの実行も許可する。
- `run` 実行時に設定ファイルが見つからない場合だけ、
  カレントディレクトリ由来の自動 ID をフォールバックとして使う。
- `set` と `get` はプロジェクトファイルを操作するため、
  鍵解決より前に必ず project root を解決する。
- `run` 実行時に親プロセスの環境変数
  `DOTENV_PRIVATE_KEY` が非空で設定されている場合は、
  その値を最優先し、OS ストア参照を行わない。
- `set` と `get` 実行時に親プロセスの環境変数
  `DOTENV_PRIVATE_KEY` が非空で設定されている場合は、
  鍵解決ではその値を最優先するが、project root 解決と設定ファイル検証は行う。
...
- v1 の CLI 出力は人間向けプレーンテキストのみとし、
  JSON 出力や静音モードは提供しない。
- `get` は要求された復号済み値を stdout に出力してよいが、
  wrapper は `DOTENV_PRIVATE_KEY` を決して出力してはならない。
...
### 7.1 `run`、`set`、`get` 時のプロジェクトルート解決

`run`、`set`、`get` は現在の作業ディレクトリから親方向へ探索し、
...
`run` は事前注入鍵がない場合だけこの探索を使う。
`set` と `get` は project file を操作するため常にこの探索を使い、
解決した project root を `cwd` にして bundled `dotenvx` を実行する。
...
- `set` と `get` の鍵ソース優先順位は、
  project context 解決、事前注入鍵、OS ストア、エラーの順とする。
- `set` と `get` では事前注入鍵があっても、
  project root 解決と設定ファイル検証を省略してはならず、
  省略してよいのは OS ストア参照だけとする。
- `init` の鍵ソース優先順位は、
  OS ストア既存値、事前注入鍵、
...
- `set` は位置引数 2 つだけを受け付ける。
- `set` に短縮コマンドはない。

#### 8.3.2 実行フロー

1. [7.1](#71-runsetget-時のプロジェクトルート解決) に従って
  project root を解決する。
2. 親プロセスの `DOTENV_PRIVATE_KEY` が非空ならその値を使う。
  そうでなければ解決した ID に対応する秘密鍵を OS ストアから取得する。
3. 現在の環境変数をコピーし、
  `DOTENV_PRIVATE_KEY` を子プロセス用に設定する。
4. 解決した project root で `dotenvx set <key> <value>` を起動する。
5. 子プロセスの終了コードまたはシグナルをそのまま返す。

#### 8.3.3 失敗時ルール

- 事前注入鍵が存在していても、設定ファイルが壊れている場合は構文エラーとして終了する。
- 事前注入鍵がなく、解決した ID の鍵が見つからない場合は、
  `init` の再実行または承認済みの別経路からの鍵注入を促す
  メッセージ付きで終了する。
- `dotenvx` の起動に失敗した場合は依存関係エラーとして終了する。
- 未対応プラットフォームまたは、事前注入鍵がない状態での
  ネイティブストア利用不可の場合は環境要件エラーとして終了する。
...
3. 現在の環境変数をコピーし、
  `DOTENV_PRIVATE_KEY` を子プロセス用に設定する。
4. 解決した project root で `dotenvx get <key>` を起動する。
5. 子プロセスの終了コードまたはシグナルをそのまま返す。

#### 8.4.3 出力と失敗時ルール

- `get` は要求された復号済み値を stdout へそのまま出力してよい。
- wrapper は `DOTENV_PRIVATE_KEY` や成功バナーを出力してはならない。
- 事前注入鍵が存在していても、設定ファイルが壊れている場合は構文エラーとして終了する。
- 事前注入鍵がなく、解決した ID の鍵が見つからない場合は、
  `init` の再実行または承認済みの別経路からの鍵注入を促す
  メッセージ付きで終了する。
...
- `DOTENV_PRIVATE_KEY` を標準出力、標準エラー、ログに出してはならない。
- `get` だけは要求された復号済み値の stdout 出力を許容するが、
  wrapper は `DOTENV_PRIVATE_KEY` を出力してはならない。
- `list` は鍵の存在確認に必要な ID 以外を出してはならない。
- `run`、`set`、`get` の環境変数注入は起動した子プロセス系統に限定する。
...
- `DOTENV_PRIVATE_KEY` が事前設定された状態で `run` を実行すると、
  ...
  その値を使って子プロセスを起動できる。
- 子ディレクトリから `set` を実行すると、
  最寄り祖先の project root にある暗号化済み `.env` を更新できる。
- `DOTENV_PRIVATE_KEY` が事前設定された状態で `get` を実行すると、
  ローカルストアを参照しなくても、最寄り project root を解決したうえで
  要求された値を出力できる。
- macOS、Windows、Linux の各対応 OS で、`init`、`run`、`set`、`get`、`list`、`remove` は同じ論理契約で振る舞う。
- `dxk init`、`dxk run -- <command> [args...]`、`dxk set <key> <value>`、`dxk get <key>`、`dxk ls`、`dxk rm <id>` は、正規名と同じ終了コードと出力契約で動作する。
- `npm pack` で生成した tarball を一時環境へ導入した場合、暗号化済みプロジェクトに対する `set` と `get` も、グローバル `dotenvx` なしで同梱依存経由で動作する。
