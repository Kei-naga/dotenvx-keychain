...
- bootstrap 失敗:
  一時ディレクトリでの `dotenvx` 実行失敗、期待した `.env` 不在、
  `.env` 反映失敗を含む実行時エラーとして終了コード `4`。
...
- `init` の新規ブートストラップで生成される `.env` は、
  親プロセス環境由来の無関係な値を含んではならない。
...
- 空プロジェクト bootstrap で `PATH` や `HOME` などの実行基盤変数が
  最終 `.env` に混入しないこと
...
- `init` で既存設定 ID 再利用
- `init` で新規プロジェクトを bootstrap できること
...
- `.env` 不在の新規プロジェクトで成功すること
- 空の `.env` で成功すること
- 既存の平文 `.env` を保持したまま、値だけ暗号化済みへ置き換えること
- bootstrap 後の `.env` に placeholder が残らないこと
- bootstrap 後の `.env` に親プロセス環境由来の値が混入しないこと
- `.env` 更新失敗や設定ファイル書き込み失敗時に、元の `.env` へ戻ること
